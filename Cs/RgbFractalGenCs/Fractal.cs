...
internal class Fractal {
	...
	internal static List<(string, CutFunction)> cutFunctions = [
		...
		("TriComb", TriComb),									// 3
		("Tetraflake_Symmetric", Tetraflake_Symmetric),			// 4
		("RadHoles", Tetraflake_Symmetric_RadHoles),			// 5
		("CornerHoles", Tetraflake_Symmetric_CornerHoles),		// 6
		("TriangleHoles", Tetraflake_Symmetric_TriangleHoles),	// 7
	...
	internal double childSize;   // Scale Of Self Similars Inside (how much to scale the image when switching parent<->child)
	...
	internal double cutSize;     // A scaling multiplier to test cutting off iterations that are completely outside the view (if you see pieces disappearing too early when zooming in, increase it, if not yo ucan decrease to boost performance)
	internal double[]
		childX;                 // X coord shifts of Self Similars Inside
	internal double[] 
		childY;                 // Y coord shifts of Self Similars Inside
	internal List<(string, double[])> 
		childAngle;				// Angle shifts of Self Similars Inside
	internal List<(string, short[])> 
	...
	private static long ApplyNoChildComplexAngle(int index, long rules, long flags, int lastRule, int lastIndex) {
		long newFlags = flags & ((long)1 << (lastIndex));
		long cb = (long)1 << lastIndex;
		if ((rules & cb) > 0)
			newFlags |= cb;
	...
	public static long NoChildAngled(int index, long inFlags, Fractal f) {
	...
	public static long NoChildNoAngled(int index, long inFlags, Fractal f) {
		int c = f.childCount - 1;
		(inFlags, var newFlags, var rules, var flags, _) = Param(inFlags, f.childCount, f.childCount + 1);
		return index > 0 
