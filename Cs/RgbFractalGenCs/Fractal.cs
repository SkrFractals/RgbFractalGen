...
internal class Fractal {
	...
	public static int FlakeTemplate(int index, int inFlags, int bits) {
		(inFlags, var newFlags, var rules, var flags, _) = Param(inFlags, bits + 1, bits + 2);
	...
	public static int Trees_NoChildParam(int index, int inFlags) {
		// Not a very good one...
		(_, var newFlags, var rules, var flags, _) = Param(inFlags, 9, 9);
		if (index == 0)
			return newFlags;
		if ((flags >> (index - 1) & 1) == 1)
			return -1;
	...
	public static int TriComb_Param(int index, int inFlags) {
		(inFlags, var newFlags, var rules, _, _) = Param(inFlags, 13, 6);
	...
	public static int Triflake_NoCornerParam(int index, int inFlags) => FlakeTemplate(index, inFlags, 3);
	...
	private static int Tetraflake_CustomCornerParam(int index, int inFlags, int fb, int fi) {
		(_, var newFlags, var rules, var flags, _) = Param(inFlags, fb, fb);
		if (index == 0)
			return newFlags;
	...
	public static int Tetraflake_NoCornerParam(int index, int inFlags) => Tetraflake_CustomCornerParam(index, inFlags, 15, 1);
	public static int Tetraflake_NoCornerRadHolesParam(int index, int inFlags) => index >= 13 ? -1 : Tetraflake_CustomCornerParam(index, inFlags, 12, 1);
	public static int Tetraflake_NoCornerCornerHolesParam(int index, int inFlags) => index is >= 10 and < 13 ? -1 : Tetraflake_CustomCornerParam(index, inFlags, 9, 1);
	public static int Tetraflake_NoCornerTriangleHolesParam(int index, int inFlags) => index is > 0 and <= 3 ? -1 : Tetraflake_CustomCornerParam(index, inFlags, 12, 4);
	#endregion

	#region CutFunctions_Carpet
	public static int Carpet_NoCornerParam(int index, int inFlags) => FlakeTemplate(index, inFlags, 8);
	...
	public static int Pentaflake_NoCornerParam(int index, int inFlags) => FlakeTemplate(index, inFlags, 5);
	...
	public static int Hexaflake_NoCornerParam(int index, int inFlags) => FlakeTemplate(index, inFlags, 6);
	...
	public static int Beamtree_NoBeam(int index, int inFlags) {
		(inFlags, var newFlags, var rules, _, _) = Param(inFlags, 9, 2);
		//if (inFlags < 0) inFlags = 0; // No params
		//int newFlags = 0;
		if (index < 4)
			return newFlags;
