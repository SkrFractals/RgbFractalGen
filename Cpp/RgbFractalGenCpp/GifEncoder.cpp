...
static void getColorMap(std::atomic_bool* cancelRequested, uint8_t* colorMap, const uint8_t* pixels, int nBytes, int quality) {
    // TODO cancel
    initnet(pixels, nBytes, quality);
    learn(cancelRequested);    // slowest
    unbiasnet();
    inxbuild();
    getcolourmap(colorMap);
}

static void getRasterBits(std::atomic_bool* cancelRequested, uint8_t* rasterBits, const uint8_t* pixels, int w, int h) {
...
bool GifEncoder::push(std::atomic_bool* cancelRequested, PixelFormat format, const uint8_t* frame, int width, int height, int delay) {
    if (m_gifFile == nullptr) {
        return false;
    }
    ...
    if (m_useGlobalColorMap) {
        ...
        getColorMap(cancelRequested, (uint8_t*)colorMap->Colors, pixels, width * height, m_quality); // slowest
        if (*cancelRequested)
            return false;

        auto* rasterBits = (GifByteType*)malloc(width * height);
        getRasterBits(cancelRequested, (uint8_t*)rasterBits, pixels, width, height); // slow
        if (*cancelRequested)
            return false;
...
bool GifEncoder::close(std::atomic_bool* cancelRequested) {
    if (m_gifFile == nullptr) {
        return false;
    }
    ...
    if (m_useGlobalColorMap) {
        ...
        getColorMap(cancelRequested, (uint8_t*)globalColorMap->Colors, m_framePixels,
        ...
        for (int i = 0; i < m_frameCount; ++i) {
            ...
            getRasterBits(cancelRequested, (uint8_t*)rasterBits, pixels, m_frameWidth, m_frameHeight);
            if (*cancelRequested)
                return false;
