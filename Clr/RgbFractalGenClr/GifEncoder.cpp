...
static void getColorMap(System::Threading::CancellationToken token, uint8_t *colorMap, const uint8_t *pixels, int nBytes, int quality) {
    // TODO cancel
...
static void getRasterBits(System::Threading::CancellationToken token, uint8_t *rasterBits, const uint8_t *pixels, int w, int h) {
...
bool GifEncoder::push(System::Threading::CancellationToken token, PixelFormat format, const uint8_t *frame, int width, int height, int delay) {
    if (m_gifFile == nullptr) {
        return false;
    }
...
//            printf("realloc 2\n");
        ...
        getColorMap(token, (uint8_t *) colorMap->Colors, pixels, width * height, m_quality); // slowest
        if (token.IsCancellationRequested)
            return false;

        auto *rasterBits = (GifByteType *) malloc(width * height);
        getRasterBits(token, (uint8_t *) rasterBits, pixels, width, height); // slow
        if (token.IsCancellationRequested)
            return false;
...
bool GifEncoder::close(System::Threading::CancellationToken token) {
    ...
    if (m_useGlobalColorMap) {
        ...
        getColorMap(token, (uint8_t *) globalColorMap->Colors, m_framePixels,
        ...
        for (int i = 0; i < m_frameCount; ++i) {
            ...
            getRasterBits(token, (uint8_t *) rasterBits, pixels, m_frameWidth, m_frameHeight);
            if (token.IsCancellationRequested)
                return false;
